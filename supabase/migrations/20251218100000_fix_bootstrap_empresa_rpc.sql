-- =============================================================================
-- Function: public.bootstrap_empresa_for_current_user(text,text)
-- Padrões: SECURITY DEFINER, search_path fixo, grants restritos
-- Notas:
--  - Usa public.current_user_id() (JWT) para resolver o usuário.
--  - Tenta ativar empresa existente; caso não exista, cria uma nova.
--  - Ajustada para nosso schema (empresas: razao_social, fantasia).
--  - Operador JSONB '?' seguro (jsonb ? text).
-- =============================================================================
...
BEGIN
    ...
    -- 3) Criar empresa nova (sem depender de campos inexistentes)
    --    (Leitura opcional de metadados do usuário; raw_user_meta_data é jsonb)
    SELECT u.raw_user_meta_data
      INTO v_user_meta
      FROM auth.users u
     WHERE u.id = v_user_id;

    -- Exemplo de checagem jsonb segura (se útil futuramente):
    ...
    -- Vincula usuário à empresa (schema real: empresa_usuarios tem empresa_id, user_id)
