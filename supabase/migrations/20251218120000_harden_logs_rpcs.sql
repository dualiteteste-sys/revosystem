-- ======================================================================
-- RPCs de Logs: SD + search_path fixo + paginação por keyset (SETOF)
-- ======================================================================

-- Fonte de dados no schema audit
...
as $$
  ...
  where occurred_at >= p_from and occurred_at <= p_to
    and (p_after is null or occurred_at < p_after)             -- keyset
...
revoke all on function audit.list_events_for_current_user(
  timestamptz,timestamptz,text[],text[],text[],text,timestamptz,int
) from public;
...
-- Wrapper público com a MESMA assinatura/retorno (conveniência PostgREST)
...
-- Sinaliza PostgREST para recarregar o schema
