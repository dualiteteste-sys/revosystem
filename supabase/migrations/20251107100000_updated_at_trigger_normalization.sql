...
BEGIN
  ...
  LOOP
    ...
    IF v_has_trigger THEN
      -- Garante que aponta para a função correta
      SELECT EXISTS (
        SELECT 1 FROM pg_trigger tg
        WHERE tg.tgrelid = v_rel
          AND tg.tgname  = 'tg_set_updated_at'
    ...
    -- Remove QUALQUER outro trigger (nome != tg_set_updated_at) que use
    -- a função correta OU a legacy (evita duplicidade).
    FOR r_drop IN
