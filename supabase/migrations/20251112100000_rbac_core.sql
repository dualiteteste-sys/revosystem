-- =====================================================================
-- RBAC Core: roles, permissions, role_permissions, overrides, vínculo
-- =====================================================================
...
-- 1) Tabelas base (globais por tenant-aware via joins; leitura controlada)
create table if not exists public.roles (
  id uuid primary key default gen_random_uuid(),
  slug text not null unique,    -- 'OWNER','ADMIN','FINANCE','OPS','READONLY'
  name text not null,
  precedence int not null default 100,  -- menor = mais alto (OWNER=0)
...
create table if not exists public.permissions (
  id uuid primary key default gen_random_uuid(),
  module text not null,         -- ex: 'usuarios','contas_a_receber','ordem_servico','produtos','logs', etc.
...
-- Overrides opcionais por usuário (granular)
create table if not exists public.user_permission_overrides (
  empresa_id uuid not null references public.empresas(id) on delete cascade,
  user_id uuid not null,
  permission_id uuid not null references public.permissions(id) on delete cascade,
  allow boolean not null, -- true=força permitir, false=força negar
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
-- 4) Sem RLS nessas tabelas "catálogo" (gerenciadas via RPCs SD). Se preferir, aplicar RLS futuramente.

-- 5) Vínculo de papel no relacionamento empresa_usuarios
--    (um papel por usuário por empresa; simples e rápido. Para M2M, criar tabela extra no futuro.)
do $$
begin
  if not exists (
    select 1
...
-- Índice auxiliar (empresa_id, role_id)
create index if not exists idx_empresa_usuarios__empresa_role on public.empresa_usuarios(empresa_id, role_id);

-- 6) Seed mínimo de papéis (idempotente)
insert into public.roles (slug, name, precedence)
values
...
-- 7) Seed mínimo de permissões (módulos principais)
-- Ajuste/expanda a lista conforme módulos habilitados
...
-- 8) Política default de role_permissions (idempotente)
-- OWNER: tudo
...
-- ADMIN: tudo exceto 'usuarios.manage' (pode gerenciar mas sem encerrar empresa/billing se atrelarmos depois)
insert into public.role_permissions(role_id, permission_id, allow)
select r.id, p.id, true
from public.roles r
join public.permissions p on true
...
-- FINANCE: contas a receber (sem delete por padrão), leitura básica de usuários (view), centros de custo (view|create|update)
insert into public.role_permissions(role_id, permission_id, allow)
select r.id, p.id, true
from public.roles r
...
-- OPS: operações (produtos/serviços e centros de custo sem delete), sem usuários
insert into public.role_permissions(role_id, permission_id, allow)
select r.id, p.id, true
from public.roles r
join public.permissions p
...
-- READONLY: somente view nos módulos operacionais e logs
insert into public.role_permissions(role_id, permission_id, allow)
select r.id, p.id, true
from public.roles r
join public.permissions p
...
-- 9) Função: resolve papel do usuário atual na empresa atual
...
-- 10) Função: checar permissão efetiva (papel + override)
...
-- 11) Garantir que exista sempre >=1 OWNER por empresa (constraint lógica via função util a ser chamada nas RPCs de gestão)
